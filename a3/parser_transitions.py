...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            newDepen = (self.stack[-1], self.stack[-2])
            del self.stack[-2]
            self.dependencies.append(newDepen)
        else:
            newDepen = (self.stack[-2], self.stack[-1])
            del self.stack[-1]
            self.dependencies.append(newDepen)
    ...
    while unfinished_parses:
        for i in range(0, len(unfinished_parses), batch_size):
            minibatch = unfinished_parses[
                i: min(i + batch_size, len(unfinished_parses))]
            transition_ravel = model.predict(minibatch)
            for parse, transition in zip(minibatch, transition_ravel):
